...
namespace WorldWind.PluginEngine
{
   ...
   public class PluginCompiler
   {
      ...
      System.Collections.Generic.List<Plugin> m_plugins = new System.Collections.Generic.List<Plugin>();
      ...
      public PluginCompiler( /* MainApplication worldWind, string pluginDirectory*/ )
      {
         ...
         // Reference all assemblies WW has a reference to
      ...
      //         pi.Name = t.Name + " (internal)";
      //         pi.IsLoadedAtStartup = true;
      ...
      public Plugin Load(string pluginPath)
      {
         if (pluginPath != null)
         {
            ...
            string extension = Path.GetExtension(pluginPath).ToLower();
            Assembly asm = null;
            if (extension == ".dll")
            {
            ...
            else
            {
               ...
               if (cdp == null)
                  return null;
               PluginInfo pi = new PluginInfo();
               pi.FullPath = pluginPath;
               asm = Compile(pi, cdp);
            }

            return GetPluginInterface(asm);
         }
         return null;

         //string pluginPath = PluginRootDirectory;
         //if( pi.FullPath != null && pi.FullPath.Length > 0)
         //   pluginPath = Path.GetDirectoryName(pi.FullPath);

         //pi.Plugin.PluginLoad(worldWindow);//, pluginPath);
      }

      public void AddPlugin(Plugin plugin, WorldWindow worldWindow, string pluginPath)
      {
         if (plugin == null || plugin.IsLoaded)
            return;
         m_plugins.Add(plugin);
         plugin.PluginLoad(worldWindow, pluginPath);
      }
      ...
      public void Dispose()
      {
         foreach (Plugin pi in m_plugins)
         {
            try
            {
               RemovePlugin(pi);
