//
// HTTPType.cs
//
// This file was generated by XMLSpy 2007r3 Enterprise Edition.
//
// YOU SHOULD NOT MODIFY THIS FILE, BECAUSE IT WILL BE
// OVERWRITTEN WHEN YOU RE-RUN CODE GENERATION.
//
// Refer to the XMLSpy Documentation for further details.
// http://www.altova.com/xmlspy
//


using System;
using System.Collections;
using System.Xml;
using Altova.Types;

namespace capabilities_1_3_0.wms
{
	public class HTTPType : Altova.Xml.Node
	{
		#region Documentation
	/*
		public static string GetAnnoDocumentation() { return ""; }
	 */ 
		#endregion

		#region Forward constructors

		public HTTPType() : base() { SetCollectionParents(); }

		public HTTPType(XmlDocument doc) : base(doc) { SetCollectionParents(); }
		public HTTPType(XmlNode node) : base(node) { SetCollectionParents(); }
		public HTTPType(Altova.Xml.Node node) : base(node) { SetCollectionParents(); }
		public HTTPType(Altova.Xml.Document doc, string namespaceURI, string prefix, string name) : base(doc, namespaceURI, prefix, name) { SetCollectionParents(); }
		#endregion // Forward constructors

		public override void AdjustPrefix()
		{

		    for (	XmlNode DOMNode = GetDomFirstChild( NodeType.Element, "http://www.opengis.net/wms", "Get" );
					DOMNode != null; 
					DOMNode = GetDomNextChild( NodeType.Element, "http://www.opengis.net/wms", "Get", DOMNode )
				)
			{
				InternalAdjustPrefix(DOMNode, true);
				new GetType(DOMNode).AdjustPrefix();
			}

		    for (	XmlNode DOMNode = GetDomFirstChild( NodeType.Element, "http://www.opengis.net/wms", "Post" );
					DOMNode != null; 
					DOMNode = GetDomNextChild( NodeType.Element, "http://www.opengis.net/wms", "Post", DOMNode )
				)
			{
				InternalAdjustPrefix(DOMNode, true);
				new PostType(DOMNode).AdjustPrefix();
			}
		}

		public void SetXsiType()
		{
 			XmlElement el = (XmlElement) domNode;
			el.SetAttribute("type", "http://www.w3.org/2001/XMLSchema-instance", "wms:HTTP");
		}


		#region Get Documentation
/*
		public static string GetGetAnnoDocumentation()
		{
			return "
        The URL prefix for the HTTP "Get" request method.
      ";		
		}
 */ 
		public static string GetGetDefault()
		{
			return "";		
		}
		#endregion

		#region Get accessor methods
		public static int GetGetMinCount()
		{
			return 1;
		}

		public static int GetMinCount
		{
			get
			{
				return 1;
			}
		}

		public static int GetGetMaxCount()
		{
			return 1;
		}

		public static int GetMaxCount
		{
			get
			{
				return 1;
			}
		}

		public int GetGetCount()
		{
			return DomChildCount(NodeType.Element, "http://www.opengis.net/wms", "Get");
		}

		public int GetCount
		{
			get
			{
				return DomChildCount(NodeType.Element, "http://www.opengis.net/wms", "Get");
			}
		}

		public bool HasGet()
		{
			return HasDomChild(NodeType.Element, "http://www.opengis.net/wms", "Get");
		}

		public GetType NewGet()
		{
			return new GetType(domNode.OwnerDocument.CreateElement("Get", "http://www.opengis.net/wms"));
		}

		public GetType GetGetAt(int index)
		{
			return new GetType(GetDomChildAt(NodeType.Element, "http://www.opengis.net/wms", "Get", index));
		}

		public XmlNode GetStartingGetCursor()
		{
			return GetDomFirstChild( NodeType.Element, "http://www.opengis.net/wms", "Get" );
		}

		public XmlNode GetAdvancedGetCursor( XmlNode curNode )
		{
			return GetDomNextChild( NodeType.Element, "http://www.opengis.net/wms", "Get", curNode );
		}

		public GetType GetGetValueAtCursor( XmlNode curNode )
		{
			if( curNode == null )
				  throw new Altova.Xml.XmlException("Out of range");
			else
				return new GetType( curNode );
		}


		public GetType GetGet()
		{
			return GetGetAt(0);
		}

		public GetType Get
		{
			get
			{
				return GetGetAt(0);
			}
		}

		public void RemoveGetAt(int index)
		{
			RemoveDomChildAt(NodeType.Element, "http://www.opengis.net/wms", "Get", index);
		}

		public void RemoveGet()
		{
			RemoveGetAt(0);
		}

		public XmlNode AddGet(GetType newValue)
		{
			return AppendDomElement("http://www.opengis.net/wms", "Get", newValue);
		}

		public void InsertGetAt(GetType newValue, int index)
		{
			InsertDomElementAt("http://www.opengis.net/wms", "Get", index, newValue);
		}

		public void ReplaceGetAt(GetType newValue, int index)
		{
			ReplaceDomElementAt("http://www.opengis.net/wms", "Get", index, newValue);
		}
		#endregion // Get accessor methods

		#region Get collection
        public GetCollection	MyGets = new GetCollection( );

        public class GetCollection: IEnumerable
        {
            HTTPType parent;
            public HTTPType Parent
			{
				set
				{
					parent = value;
				}
			}
			public GetEnumerator GetEnumerator() 
			{
				return new GetEnumerator(parent);
			}
		
			IEnumerator IEnumerable.GetEnumerator() 
			{
				return GetEnumerator();
			}
        }

        public class GetEnumerator: IEnumerator 
        {
			int nIndex;
			HTTPType parent;
			public GetEnumerator(HTTPType par) 
			{
				parent = par;
				nIndex = -1;
			}
			public void Reset() 
			{
				nIndex = -1;
			}
			public bool MoveNext() 
			{
				nIndex++;
				return(nIndex < parent.GetCount );
			}
			public GetType  Current 
			{
				get 
				{
					return(parent.GetGetAt(nIndex));
				}
			}
			object IEnumerator.Current 
			{
				get 
				{
					return(Current);
				}
			}
    	}

        #endregion // Get collection

		#region Post Documentation
/*
		public static string GetPostAnnoDocumentation()
		{
			return "
        The URL prefix for the HTTP "Post" request method.
      ";		
		}
 */ 
		public static string GetPostDefault()
		{
			return "";		
		}
		#endregion

		#region Post accessor methods
		public static int GetPostMinCount()
		{
			return 0;
		}

		public static int PostMinCount
		{
			get
			{
				return 0;
			}
		}

		public static int GetPostMaxCount()
		{
			return 1;
		}

		public static int PostMaxCount
		{
			get
			{
				return 1;
			}
		}

		public int GetPostCount()
		{
			return DomChildCount(NodeType.Element, "http://www.opengis.net/wms", "Post");
		}

		public int PostCount
		{
			get
			{
				return DomChildCount(NodeType.Element, "http://www.opengis.net/wms", "Post");
			}
		}

		public bool HasPost()
		{
			return HasDomChild(NodeType.Element, "http://www.opengis.net/wms", "Post");
		}

		public PostType NewPost()
		{
			return new PostType(domNode.OwnerDocument.CreateElement("Post", "http://www.opengis.net/wms"));
		}

		public PostType GetPostAt(int index)
		{
			return new PostType(GetDomChildAt(NodeType.Element, "http://www.opengis.net/wms", "Post", index));
		}

		public XmlNode GetStartingPostCursor()
		{
			return GetDomFirstChild( NodeType.Element, "http://www.opengis.net/wms", "Post" );
		}

		public XmlNode GetAdvancedPostCursor( XmlNode curNode )
		{
			return GetDomNextChild( NodeType.Element, "http://www.opengis.net/wms", "Post", curNode );
		}

		public PostType GetPostValueAtCursor( XmlNode curNode )
		{
			if( curNode == null )
				  throw new Altova.Xml.XmlException("Out of range");
			else
				return new PostType( curNode );
		}


		public PostType GetPost()
		{
			return GetPostAt(0);
		}

		public PostType Post
		{
			get
			{
				return GetPostAt(0);
			}
		}

		public void RemovePostAt(int index)
		{
			RemoveDomChildAt(NodeType.Element, "http://www.opengis.net/wms", "Post", index);
		}

		public void RemovePost()
		{
			RemovePostAt(0);
		}

		public XmlNode AddPost(PostType newValue)
		{
			return AppendDomElement("http://www.opengis.net/wms", "Post", newValue);
		}

		public void InsertPostAt(PostType newValue, int index)
		{
			InsertDomElementAt("http://www.opengis.net/wms", "Post", index, newValue);
		}

		public void ReplacePostAt(PostType newValue, int index)
		{
			ReplaceDomElementAt("http://www.opengis.net/wms", "Post", index, newValue);
		}
		#endregion // Post accessor methods

		#region Post collection
        public PostCollection	MyPosts = new PostCollection( );

        public class PostCollection: IEnumerable
        {
            HTTPType parent;
            public HTTPType Parent
			{
				set
				{
					parent = value;
				}
			}
			public PostEnumerator GetEnumerator() 
			{
				return new PostEnumerator(parent);
			}
		
			IEnumerator IEnumerable.GetEnumerator() 
			{
				return GetEnumerator();
			}
        }

        public class PostEnumerator: IEnumerator 
        {
			int nIndex;
			HTTPType parent;
			public PostEnumerator(HTTPType par) 
			{
				parent = par;
				nIndex = -1;
			}
			public void Reset() 
			{
				nIndex = -1;
			}
			public bool MoveNext() 
			{
				nIndex++;
				return(nIndex < parent.PostCount );
			}
			public PostType  Current 
			{
				get 
				{
					return(parent.GetPostAt(nIndex));
				}
			}
			object IEnumerator.Current 
			{
				get 
				{
					return(Current);
				}
			}
    	}

        #endregion // Post collection

        private void SetCollectionParents()
        {
            MyGets.Parent = this; 
            MyPosts.Parent = this; 
	}
}
}
