	...
	public class AtmosphereLayer : RenderableObject
	{
		...
		public override void Render(DrawArgs drawArgs)
		{
         ...
         Point3d perp2 = Point3d.cross(perp1, camera.Position);
			...
			double radius = camera.WorldRadius*1.05;
			perp1 *= radius;
			perp2 *= radius;
         ...
         borderVertices[1].X = (float)ur.X;
         borderVertices[1].Y = (float)ur.Y;
         borderVertices[1].Z = (float)ur.Z;
			borderVertices[1].Tu = 1;
			borderVertices[1].Tv = 1;

			ur = -perp1 - perp2 + offset;
         borderVertices[2].X = (float)ur.X;
         borderVertices[2].Y = (float)ur.Y;
         borderVertices[2].Z = (float)ur.Z;
			borderVertices[2].Tu = 0;
			borderVertices[2].Tv = 0;

			// Triangle 2 (uses previous 2 vertices as first 2 points (TriangleStrip))
			ur = perp1 - perp2 + offset;
         borderVertices[3].X = (float)ur.X;
         borderVertices[3].Y = (float)ur.Y;
         borderVertices[3].Z = (float)ur.Z;
