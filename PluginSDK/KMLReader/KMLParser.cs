...
namespace WorldWind.KMLReader
{
	public class KMLParser
	{
		...
		public static string KmlDataDirectory;
		public static string KmlCacheDirectory;
				...
				try
				{
					...
					else
					{
						...
						else
						{
							...
							string pal3Path = Path.Combine(KmlDataDirectory, "icons/palette-3.png");
		...
		private Style GetStyle(XmlNode style, Style oldStyle, string KmlPath)
		{
			try
			{
				...
				if (hrefNode != null)
				{
					...
					{
						filename = Path.Combine(KmlCacheDirectory, filename.Remove(0, 7));
					...
					else if (File.Exists(Path.Combine(KmlCacheDirectory, filename)))					// Use a file from disk
					{
						overStyle.NormalIcon = GetDiskImage(Path.Combine(KmlCacheDirectory, filename));
					}
				}
				...
				if (iconStyleNode != null)
				{
					...
					if (iconElementNode != null)
					{
						...
						if (iconElementHrefNode != null)
						{
							...
							{
								iconElement.href = GetDiskImage(Path.Combine(KmlCacheDirectory, filename));
							}
						}
