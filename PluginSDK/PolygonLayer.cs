...
namespace WorldWind.Renderable
{
	...
	public class PolygonLayer : RenderableObject
	{
		...
		public PolygonLayer(
			...
			System.Drawing.Color color) : base(name, parentWorld.Position, Quaternion4d.RotationYawPitchRoll(0, 0, 0))
		{
		...
		public override void Initialize(DrawArgs drawArgs)
		{
			...
			using( BinaryReader polygonFileReader = new BinaryReader(polygonFileStream, System.Text.Encoding.ASCII) ) {
				...
				for(int i = 0; i < nodeCount; i++) {
					...
					Point3d curNode = MathEngine.SphericalToCartesian(lat, lon, (this._parentWorld.EquatorialRadius + this._distanceAboveSurface));
					this._vertices[i].X = (float)curNode.X;
               this._vertices[i].Y = (float)curNode.Y;
               this._vertices[i].Z = (float)curNode.Z;
