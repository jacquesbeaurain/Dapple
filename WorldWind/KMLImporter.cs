...
namespace KMLPlugin
{
	...
	public class KMLImporter : WorldWind.PluginEngine.Plugin
	{
		...
		{
			KMLParser.KmlDataDirectory = Path.Combine(MainApplication.Settings.DataPath, "kml");
			KMLParser.KmlCacheDirectory = Path.Combine(MainApplication.Settings.CachePath, "kml");
			Directory.CreateDirectory(KMLParser.KmlCacheDirectory);

			// Load settings
			Settings.LoadSettings(Path.Combine(MainApplication.CurrentSettingsDirectory, "KMLImporter.xml"));
			...
			bool bEnabled = Napalm.NapalmIsEnabled(KMLParser.KmlCacheDirectory);
			if (bEnabled)
		...
		public override void Unload()
		{
			...
			// Save settings
			Settings.SaveSettings(Path.Combine(MainApplication.CurrentSettingsDirectory, "KMLImporter.xml"));
			...
			*/
			try
			{
				...
				if (Directory.Exists(Path.Combine(KMLParser.KmlCacheDirectory, "kmz")))
					Directory.Delete(Path.Combine(KMLParser.KmlCacheDirectory, "kmz"), true);

				foreach (string kmlfile in Directory.GetFiles(KMLParser.KmlCacheDirectory, "*.kml"))
				{
					try
					{
						File.Delete(kmlfile);
				...
				foreach (string kmzfile in Directory.GetFiles(KMLParser.KmlCacheDirectory, "*.kmz"))
				{
					try
					{
						File.Delete(kmzfile);
		...
		private void Spawn_LoadKML(string path)
		{
			...
			Napalm.Update(KMLParser.KmlCacheDirectory, version);
		}
				...
				if (kml != null)
				{
					...
					catch (Exception ex)
					{
						...
						MessageBox.Show(
							 ...
							 base.ParentApplication.RightToLeft == RightToLeft.Yes ? MessageBoxOptions.RtlReading : MessageBoxOptions.ServiceNotification);
					}
				}
			}
		...
		{
			bool bEnabled = Napalm.NapalmChangeStatus(KMLParser.KmlCacheDirectory, napalmMenuItem.Text.StartsWith("Enable"));
			if (bEnabled)
				napalmMenuItem.Text = "Disable KMLImporter autoupdate";
			else
				napalmMenuItem.Text = "Enable KMLImporter autoupdate";
	...
	public class WaitMessage : RenderableObject
	{
		...
		internal WaitMessage()
			: base("KML WaitMessage", Point3d.Empty, Quaternion4d.Identity)
		{
