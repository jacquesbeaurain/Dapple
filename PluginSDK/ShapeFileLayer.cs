		...
		public override void Render(DrawArgs drawArgs)
		{
			...
			try
			{
				...
				if (m_PointList.Count > 0)
				{
					...
					Point3d camPoint = MathEngine.SphericalToCartesian(drawArgs.WorldCamera.Latitude.Degrees, drawArgs.WorldCamera.Longitude.Degrees, m_ShapeTileArgs.LayerRadius);

					drawArgs.device.Transform.World = Matrix.Translation(-referenceCenter.Vector3);
					foreach (Point3d v in m_PointList)
					{
						if ((v - camPoint).Length < m_ShapeTileArgs.LayerRadius)
						{
				...
				if (m_LabelList.Count > 0)
				{
					...
					{
						Point3d cartesianPoint = MathEngine.SphericalToCartesian(p.Y, p.X, drawArgs.WorldCamera.WorldRadius + drawArgs.WorldCamera.TerrainElevation);
		...
		private void loadZippedShapeFile(string shapeFilePath)
		{
			...
			while (bytesRead < shpEntry.Size)
			{
				...
				if (!ignoreRecord)
				{
					...
					{
						foreach (Point3d v in pendingPoints)
							m_PointList.Add(v);
					}
				}
