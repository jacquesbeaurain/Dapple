...
namespace NASA.Plugins
{
	...
	public class BMNG : System.Windows.Forms.Form
	{
		...
		WorldWind.Renderable.RenderableObjectList m_UnShadedList = null;

		WorldWind.Renderable.ImageLayer m_BlueMarbleBase = null;
		WorldWind.Renderable.QuadTileSet m_BlueMarbleTiled = null;
		WorldWind.Renderable.RenderableObjectList m_RenderableList = new WorldWind.Renderable.RenderableObjectList("4 - The Blue Marble");
       ...
       private string m_strPluginPath;
		...
		public BMNG(string pluginPath, WorldWind.WorldWindow worldWindow)
		{
         ...
         //
			...
			comboBoxBmngVersion.SelectedIndex = 1;
			try
			{
             using (StreamReader reader = new StreamReader(pluginPath + "\\Plugins\\BlueMarble\\settings.txt"))
				{
					int lastSelectedDatasetIndex = int.Parse(reader.ReadLine().Trim());
			...
			m_BlueMarbleBase = new WorldWind.Renderable.ImageLayer(
				"0 - Blue Marble Base Image",
				m_WorldWindow.CurrentWorld,
				0,
				null,
				-90, 90, -180, 180, 255, null);
			...
			m_BlueMarbleTiled = new WorldWind.Renderable.QuadTileSet(
				"1 - Blue Marble Tiled",
            new WorldWind.GeographicBoundingBox(90, -90, -180, 180),
            m_WorldWindow.CurrentWorld,
            0,
				m_WorldWindow.CurrentWorld.TerrainAccessor,
				new WorldWind.ImageAccessor( null, 512, 36, 4, "jpg",
				String.Format("{0}\\Blue Marble", m_WorldWindow.Cache.CacheDirectory),
				new WorldWind.ImageTileService("106", "http://nww.terraserver-usa.com/nwwtile.ashx")
            )
            , 255, false);

			m_BlueMarbleList = new WorldWind.Renderable.RenderableObjectList("0 - Blue Marble");
			m_BlueMarbleList.IsOn = false;
			m_BlueMarbleList.Add(m_BlueMarbleBase);
			m_BlueMarbleList.Add(m_BlueMarbleTiled);

			m_ShadedList = new WorldWind.Renderable.RenderableObjectList("1 - BMNG");
			...
			for(int i = 0; i < 12; i++)
			{
				m_ImageLayers[0, i] = new WorldWind.Renderable.ImageLayer(
					String.Format("1 - Base Image - {0}.2004", i+1),
					m_WorldWindow.CurrentWorld,
					0,
					null,
					-90, 90, -180, 180, 255, null);
				...
				m_QuadTileLayers[0, i] = new WorldWind.Renderable.QuadTileSet(
					String.Format("0 - Tiled - {0}.2004", i+1),
					new WorldWind.GeographicBoundingBox( 90, -90, -180, 180),
               m_WorldWindow.CurrentWorld, 0,
               m_WorldWindow.CurrentWorld.TerrainAccessor,
					...
					String.Format("bmng.topo.2004{0:D2}", i+1), "http://worldwind25.arc.nasa.gov/tile/tile.aspx")
					)
               , 255, false);
			...
			m_ShadedBathyList = new WorldWind.Renderable.RenderableObjectList("2 - BMNG (Bathymetry)");
			...
			for(int i = 0; i < 12; i++)
			{
				m_ImageLayers[1, i] = new WorldWind.Renderable.ImageLayer(
					...
					m_WorldWindow.CurrentWorld,
					0,
					String.Format("{0}\\Data\\Earth\\BmngBathy\\world.topo.bathy.2004{1:D2}.jpg", pluginPath, i+1),
					-90, 90, -180, 180, 255, null);
			...
			//	m_ImageLayers[1, i].ImageUrl = String.Format("{0}world.topo.bathy.2004{1:D2}.jpg", m_BmngBaseImageUrl, i+1);
				...
				m_QuadTileLayers[1, i] = new WorldWind.Renderable.QuadTileSet(
                  ...
                  m_WorldWindow.CurrentWorld.TerrainAccessor,
						new WorldWind.ImageAccessor(
						null, 512, 36, 5, "jpg",
						String.Format("{0}\\BMNG\\{1}", m_WorldWindow.Cache.CacheDirectory, String.Format("BMNG (Shaded + Bathymetry) Tiled - {0}.2004", i+1)),
						new WorldWind.ImageTileService(
						String.Format("bmng.topo.bathy.2004{0:D2}", i+1), "http://worldwind25.arc.nasa.gov/tile/tile.aspx")
						)
                  , 255, false);
		...
		/*	m_UnShadedList = new WorldWind.Renderable.RenderableObjectList("BMNG (UnShaded)");
			...
			for(int i = 0; i < 12; i++)
			{
				m_ImageLayers[2, i] = new WorldWind.Renderable.ImageLayer(
					String.Format("1 - Base Image - {0}.2004 un", i+1),
					m_WorldWindow.CurrentWorld,
					0,
					null,
		...
		private void menuItemAbout_Click(object sender, System.EventArgs e)
		{
			...
			m_BmngAboutDialog.ShowDialog(this);
		...
		{
			using(StreamWriter sw = new StreamWriter(m_strPluginPath + "\\Plugins\\BlueMarble\\settings.txt"))
			{
	...
	public class BmngLoader : WorldWind.PluginEngine.Plugin
	{
       ...
       public BmngLoader(string pluginPath)
       {
           m_strPluginPath = pluginPath;
		...
		{
			if(WorldWindow.CurrentWorld.Name.IndexOf("Earth") >= 0)
			{
				m_BmngForm = new BMNG(m_strPluginPath, WorldWindow);
