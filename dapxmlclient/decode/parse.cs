...
namespace Geosoft.Dap.Xml
{
	...
	public class Parse
	{
		...
		public void DataSet(System.Xml.XmlNode hDataSetNode, out DataSet hDataSet)
		{
			...
			System.Xml.XmlNode hStylesheet = hDataSetNode.Attributes.GetNamedItem(Constant.Attribute.STYLESHEET_ATTR);
			...
			if (hStylesheet != null) hDataSet.Stylesheet = hStylesheet.Value;
		}
		...
		/// <summary>
		/// Parse the extract file response.
		/// </summary>
		/// <param name="hDocument">The GeosoftXML response</param>
		/// <param name="strFileName">The filename</param>
		public void ExtractFileName(System.Xml.XmlDocument hDocument, out string strFileName)
		{
			System.Xml.XmlNode hNode;
			System.Xml.XmlNode hAttr;
			...
			try
			{
				...
				hNode = hDocument.SelectSingleNode("/" + Constant.Tag.GEO_XML_TAG + "/" + Constant.Tag.RESPONSE_TAG + "/" + Constant.Tag.EXTRACT_FILE_TAG);

				if (hNode == null) throw new DapException("No file found in extract");

				hAttr = hNode.Attributes.GetNamedItem(Constant.Attribute.NAME_ATTR);

				if (hAttr == null) throw new DapException("Missing value attribute in cound element");
				strFileName = hAttr.Value;
			}
			catch (Exception e)
			{
				throw new DapException("Error retrieving filename of extracted datasets", e);
			}
		}
