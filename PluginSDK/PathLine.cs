		...
		public void Load(DrawArgs drawArgs)
		{
			...
			using( FileStream fs = new FileStream(inFile.FullName, FileMode.Open, FileAccess.Read, FileShare.Read) ) 
			{
				...
				using( BinaryReader br = new BinaryReader(ms, System.Text.Encoding.ASCII) ) 
				{
					...
					for(int i = 0; i < numCoords; i++) 
					{
						Point3d v = MathEngine.SphericalToCartesian(this.sphericalCoordinates[i].X,this.sphericalCoordinates[i].Y, this._parentWorld.EquatorialRadius + World.Settings.VerticalExaggeration * this.heightAboveSurface);
						this.linePoints[i].X = (float)v.X;
                  this.linePoints[i].Y = (float)v.Y;
                  this.linePoints[i].Z = (float)v.Z;
...
/*			if(terrainMapped)
			...
			if(newPoints == null)
			{
				...
				Point3d newPoint = MathEngine.SphericalToCartesian(latitude, longitude, this._parentWorld.EquatorialRadius + this.verticalExaggeration * heightAboveSurface );
				newLine[newLine.Length - 1].Color = this.lineColor;
            newLine[newLine.Length - 1] = new Microsoft.DirectX.Direct3D.CustomVertex.PositionColored((float)newPoint.X, (float)newPoint.Y, (float)newPoint.Z, System.Drawing.Color.Red.ToArgb());
			...
			{
            foreach (Point3d newPoint in newPoints)
				{
					CustomVertex.PositionColored[] newLine = new CustomVertex.PositionColored[this.linePoints.Length+1];
					this.linePoints.CopyTo(newLine, 0);
