		...
		public override void Initialize(DrawArgs drawArgs)
		{
			...
			using( BinaryReader boundaryFileReader = new BinaryReader(boundaryFileStream, System.Text.Encoding.ASCII) ) {
				...
				for(int i = 0; i < count; i++) {
					...
					Point3d v = MathEngine.SphericalToCartesian(lat, lon, (this._parentWorld.EquatorialRadius + this._distanceAboveSurface));
					this.vertices[i].X = (float)v.X;
               this.vertices[i].Y = (float)v.Y;
               this.vertices[i].Z = (float)v.Z;
